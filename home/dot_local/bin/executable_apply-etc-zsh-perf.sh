#!/usr/bin/env bash
# Cache the base image's eager /etc/zsh version-manager init, which cost 874ms of a 1018ms
# tmux pane. The AMI runs `goenv init -`, `pyenv init -` and sources nvm.sh on every shell,
# and /etc/zsh/zshrc re-sources all of /etc/zsh/zprofile.d even for login shells that
# /etc/zprofile already covered.
#
# Almost all of that output is deterministic, so it is cached rather than re-derived. The
# goenv()/pyenv() functions and the pyenv-virtualenv precmd hook survive; goenv's automatic
# rehash and nvm's eager load do not.
#
# /etc is wiped on a host restart, so ~/personalize re-runs this at startup, as it does for
# apply-sshd-keepalive.sh. Baked versions are resolved at generation time -- re-run by hand
# after switching a Go/Python/node version.
#
