...
Homebrew on macOS, upstream's prebuilt release into `~/.local/bin` on
Debian/Ubuntu), `eval`'d in `02-zinit`, so it's not a zinit-managed plugin. Right/End accept the
